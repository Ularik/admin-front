...
import { toast } from "sonner"; // Или любая ваша библиотека тостов
import {
  ArrowLeft,
  Send,
  Paperclip,
...
export default function CreateTaskReplyPage() {
  ...
  // Используем mutate, isPending и error из хука TanStack Query
  const { mutate: createReply, isPending, error } = useReplyCreate();
  ...
  const onSubmit = (data: ReplyCreateType) => {
    createReply(
      ...
      {
        onSuccess: () => {
          toast.success("Успешно отправили ответ на задачу", {
            position: "top-center",
          });
          router.back();
        },
  ...
  // Извлекаем сообщение об ошибке из Axios / TanStack Query
  const serverErrorMessage =
    (error as any)?.response?.data?.detail || error?.message || null;
